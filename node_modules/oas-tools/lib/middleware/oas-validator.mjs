...
function locationFormat(inProperty) {
  ...
  return dict[inProperty]; //return (inProperty == "path" ? "params" : inProperty); //TODO: if only 'path' changes then this is the solution!
}
/**
...
function getParameterValue(req, parameter) {
  ...
  var val; // Get the value to validate based on the operation parameter type
...
function convertValue(value, optionalSchema, optionalType) {
  ...
  const schema = optionalSchema === undefined ? {} : optionalSchema; // If there is an empty value and allowEmptyValue is true, return it
